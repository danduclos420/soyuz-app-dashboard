...
const LIE_OPTIONS = [
  ...
  { 
    ...
    angle: 45,
    indicator: 'center',
    idealFor: 'Standard Stance'
  },
  { 
    id: 'lie-5-5', 
    label: 'LIE 5.5', 
    subtitle: 'Aggressive Stance',
    description: 'Slightly more upright. Better for puck control in tight and quick releases.',
    angle: 46.5,
    indicator: 'center',
    idealFor: 'Quick Handlers'
  },
  { 
    id: 'lie-6', 
    label: 'LIE 6', 
    ...
    angle: 48,
    indicator: 'toe',
    idealFor: 'Upright Stance Skaters'
  }
];
          ...
          <div className="space-y-8 lg:space-y-12">
            <div>
              ...
              <p className="text-[#888888] text-xs sm:text-sm uppercase tracking-widest font-bold max-w-md">
                Find the perfect blade angle for your skating style and puck handling preference.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3">
              {LIE_OPTIONS.map((lie) => (
                <button
                  ...
                  className={`p-4 sm:p-6 border text-left transition-all duration-300 ${
                    activeLie.id === lie.id ? 'bg-white/[0.04] border-soyuz' : 'bg-white/[0.01] border-white/5 hover:border-white/10'
                  }`}
                >
                  <div className="flex justify-between items-center mb-2">
                     <h3 className={`text-xl lg:text-3xl font-display italic ${activeLie.id === lie.id ? 'text-white' : 'text-white/20'}`}>
                        {lie.label}
                     </h3>
                     {activeLie.id === lie.id && <Target size={14} className="text-soyuz pulse-slow" />}
                  </div>
                  <p className={`text-[8px] lg:text-[9px] font-black uppercase tracking-[0.2em] ${activeLie.id === lie.id ? 'text-soyuz' : 'text-[#333]'}`}>
                    {lie.subtitle}
                  </p>
                </button>
              ))}
          ...
          <div className="lg:sticky lg:top-32 space-y-8">
            <div className="aspect-square bg-[#050505] border border-white/5 rounded-3xl p-1 relative overflow-hidden flex flex-col items-center justify-center">
               ...
               <svg viewBox="0 0 400 400" className="w-full h-full">
                  ...
                  <motion.g 
                    initial={false}
                    animate={{ rotate: 50 - activeLie.angle }}
                  ...
                  >
                     ...
                     <motion.circle 
                        ...
                        cy="350" r="15" fill="#CC0000" className="opacity-20 blur-[10px]"
                     />
                  </motion.g>
               ...
               <div className="absolute bottom-8 inset-x-8 flex justify-between">
                  <div>
                     ...
                     <span className="text-xl font-display italic text-soyuz leading-none">{activeLie.label}</span>
                  </div>
                  <div className="text-right">
                     <span className="text-[8px] font-black text-white/20 uppercase tracking-[0.4em] block mb-1">Diagnostics</span>
                     <span className="text-xl font-display italic text-white leading-none uppercase">STABLE</span>
                  </div>
               </div>
            </div>
